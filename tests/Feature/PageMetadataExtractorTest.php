...
it('truncates very long titles', function () {
    ...
    expect(mb_strlen($this->extractor->extractTitle($html)))->toBe(255);
});
...
it('extracts meta description', function () {
    ...
    expect($this->extractor->extractDescription($html))->toBe('A powerful AI tool for developers');
});
...
it('extracts meta description with content before name', function () {
    ...
    expect($this->extractor->extractDescription($html))->toBe('Build with AI');
});

it('returns null for missing meta description', function () {
    $html = '<html><head><title>Test</title></head></html>';

    expect($this->extractor->extractDescription($html))->toBeNull();
});
...
it('decodes html entities in description', function () {
    ...
    expect($this->extractor->extractDescription($html))->toBe('AI & ML — The Future');
});
